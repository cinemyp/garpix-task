...
class MyMap extends React.Component {
  ...
  addRoute = () => {
    if (this.ymaps && this.map) {
      this.ymaps
        .route(["Иваново,улица 8 Марта, 32", "Москва, Большая Татарская улица, 21"], {
          multiRoute: true
        })
        .then(route => {
          this.route = route;
  ...
  render() {
    return (
      <div className={style["mapoblast_map"]}>
          ...
          <YMaps query={{ apikey: '6dbb0668-7b63-45f5-a425-49bf1a4439b1' }}  onApiAvaliable={ymaps => this.handleApiAvaliable(ymaps)}>
            <Map
              state={mapState}
